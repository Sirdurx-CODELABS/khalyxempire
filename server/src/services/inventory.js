...
export async function sellStock(items, { orderId, channel = 'pos' } = {}) {
  const sold = [];
  try {
    for (const item of items) {
      const updated = await Product.findOneAndUpdate(
...
export async function adjustStock({ productId, variantId, qty, note = 'Manual adjustment' }) {
  ...
  await InventoryMovement.create({
    type: delta > 0 ? 'receive' : 'adjustment',
    channel: 'system',
    product: productId,
    variantId,
    sku: variant?.sku,
